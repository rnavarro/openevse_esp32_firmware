...
- **ArduinoMongoose stays a vendored-Mongoose dependency regardless.** `jeremypoulter/ArduinoMongoose` is *not* a Cesanta fork — it's an original Arduino **wrapper** (created 2019, `isFork: false`) that **vendors** a bundled copy of Cesanta's amalgamation, currently **Mongoose 6.18** (the last 6.x release, EOL ~Feb 2021). IDF5 doesn't change that. Moving to Mongoose 7.x is a ground-up API rewrite (the wrapper's whole class model rides the 6.x `mg_connection` / `MG_EV_*` API) and would also force **MicroOcppMongoose** to change — OCPP is coupled to the 6.x Mongoose API, visible as the `MO_MG_VERSION_614` compat define in `platformio.ini`. So a M7 move is a two-library, cross-maintainer rewrite, out of scope for our IPv6 work (and M7 being natively dual-stack would retire our 6.x patches anyway). The Mongoose-version question never appears in the OpenEVSE Discord (a support server); the definitive "why 6.x" would need a GitHub issue or asking Jez.
