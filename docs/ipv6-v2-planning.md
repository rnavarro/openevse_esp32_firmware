...
- **ArduinoMongoose stays a vendored-Mongoose dependency regardless.** `jeremypoulter/ArduinoMongoose` is *not* a Cesanta fork — it's an original Arduino **wrapper** (created 2019, `isFork: false`) that **vendors** a bundled copy of Cesanta's amalgamation, currently **Mongoose 6.18** (the last 6.x release, EOL ~Feb 2021). IDF5 doesn't change that, and M7 being natively dual-stack would retire our 6.x IPv6 patches anyway. The Mongoose-version question never appears in the OpenEVSE Discord (a support server); the definitive "why 6.x" would need a GitHub issue or asking Jez.
  - A Mongoose-7 migration (which would retire our IPv6 patches) is a **separate initiative scoped in `docs/mongoose7-migration-scoping.md`** — note that OCPP is *not* the blocker (`matth-x/MicroOcppMongoose` already supports M7), so the real scope is the ArduinoMongoose wrapper + firmware. Sequence it after IPv6-on-core-3.
...
6. **Mongoose 7 migration** — a *separate initiative*, scoped in **`docs/mongoose7-migration-scoping.md`**. Relevant here only because M7 is natively dual-stack and would retire our IPv6 patches, so **sequence M7 after landing IPv6 on core-3** (don't block a validated, in-flight feature on a speculative rewrite). Not a v2-IPv6 dependency.
...
- **Mongoose-7 split into its own doc (2026-06-08):** M7 is not technically IPv6 work, so it now lives in `docs/mongoose7-migration-scoping.md` (scoping pass: wrapper API surface, firmware call sites, 6→7 delta, safety boundary, effort/risk). Recorded here only as a sequencing constraint (M7 after IPv6-on-core-3). Earlier "two-library cross-maintainer rewrite" claim was corrected — OCPP/MicroOcpp is already M7-ready.
