...
- **ArduinoMongoose stays a vendored-Mongoose dependency regardless.** `jeremypoulter/ArduinoMongoose` is *not* a Cesanta fork — it's an original Arduino **wrapper** (created 2019, `isFork: false`) that **vendors** a bundled copy of Cesanta's amalgamation, currently **Mongoose 6.18** (the last 6.x release, EOL ~Feb 2021). IDF5 doesn't change that, and M7 being natively dual-stack would retire our 6.x IPv6 patches anyway. The Mongoose-version question never appears in the OpenEVSE Discord (a support server); the definitive "why 6.x" would need a GitHub issue or asking Jez.
  - **Correction (2026-06-08):** an earlier draft of this line claimed a Mongoose-7 move was a "two-library, cross-maintainer rewrite" because OCPP was "coupled to the 6.x API." Primary evidence says otherwise: **`matth-x/MicroOcppMongoose` already supports Mongoose 7** (README lists v6.14 / v7.8 / v7.13–v7.17, selected via `MO_MG_USE_VERSION`; its source carries both 6.x and 7.x `MG_EV_*` code paths). The `MO_MG_VERSION_614` flag in our `platformio.ini` is a *selectable* version, not a lock. So OCPP is **not** the M7 blocker — the real scope of a M7 migration is rewriting `jeremypoulter/ArduinoMongoose`'s wrapper classes onto the 7.x API plus updating the firmware's call sites (one library + the firmware, gated on Jez/Chris buy-in), not a third-party-maintainer problem. See the dedicated M7 note in Open questions.
...
6. **Mongoose 7 migration (parked — "propose, don't surprise").** The next EOL shoe after IDF: the bundled Mongoose 6.18 is EOL. M7 is natively dual-stack + modern TLS + simpler API, and would retire our IPv6 patches. Feasibility is *better* than first assessed (OCPP/MicroOcpp is already M7-ready — see the correction above), so the real scope is the `jeremypoulter/ArduinoMongoose` wrapper rewrite (its HTTP server / MQTT / HTTP client / WebSocket classes ride the 6.x `MG_EV_*` model) + the firmware's call sites. Before any code: (a) inventory the wrapper's public API surface and the firmware call sites; (b) measure the 6.x→7.x API delta; (c) float a design issue with Jez (owns the wrapper) + Chris (release lead) to gauge appetite. Sequence it *after* landing IPv6 on core-3 — don't couple a validated, in-flight feature to a speculative rewrite. This is a candidate larger contribution to an under-resourced project, not a v2-IPv6 dependency.
...
- **v2 scope decision parked (2026-06-08):** operator chose to **wait for Chris's core-2 deprecation answer** before declaring v2 "core-3-only vs dual-track" in this doc. Operator is migrating to core-3 regardless; Chris's answer only affects whether *upstream* keeps core-2 alive for others (i.e. whether "Track A" — upstreaming our IDF4 work for core-2 builds — is worth it). The **dev-board gate is resolved**: new 16MB ESP32 boards arrive week of 2026-06-08 and become the v2 metal-verification platform; the 4MB Olimex retires from the v2 path (it can't run a core-3 target — no 4MB core-3 env in #1091's stack).
- **Mongoose-7 feasibility re-scoped (2026-06-08):** corrected the earlier "two-library cross-maintainer rewrite" claim — `matth-x/MicroOcppMongoose` already supports M7 (v6.14 / v7.8–v7.17 via `MO_MG_USE_VERSION`), so OCPP isn't the blocker; the real M7 scope is the ArduinoMongoose wrapper + firmware call sites. Parked as a candidate larger contribution (Open questions #6).
