...
**Crash found during C2 — boot-time reboot loop when a non-routable GUA wins slot 0 (root cause confirmed and fixed, 2026-06-07).** When `radvd` was left running *before* the device associated, the device caught the `3fff` RA during boot SLAAC, the slot-swap promoted `3fff` into slot 0, and ~5 s later `loopTask` tripped the task watchdog (`abort()` → `SW_CPU_RESET`), repeating every cycle. Reproduced on both pre-refactor `55c2380d` and refactor `4997a79b` (the win is a non-deterministic boot SLAAC race; with aggressive radvd it reproduced within a couple of restarts). Fixed in `731c0ae` (`fix/mqtt-dns-blocking-watchdog`).

**Root cause (confirmed).** `Mqtt::attemptConnection()` pre-resolves the broker with synchronous `getaddrinfo()` on the cold-cache path (`src/mqtt.cpp`). LwIP DNS is retry-bounded (~14 s per family, ~28 s for the AAAA+A pair) but that exceeds the 5 s task watchdog, so a cold resolve that stalls trips the `loopTask` watchdog and reboots. The fact chain: every crash cycle's last line before the hang is `Mongoose DNS nameserver set to: [fd00:…:0002]` with **no** following `MQTT resolved …` line (the resolve never completed); non-crashing cycles differ only in that a routable `2602` held slot 0, the broker resolved in ~30 ms, and `3fff` arrived later as a *secondary* (no swap). After the fix the same boot logs `MQTT getaddrinfo(...) IPv4 failed: 202` (EAI_FAIL) and the device stays up — direct confirmation that the blocking `getaddrinfo()` was the stall.

**Why a non-routable slot-0 winner stalls the resolve (strongly inferred, not packet-confirmed).** The mDNS slot-0 swap physically reorders the LwIP `ip6_addr[]` table, and LwIP's source-address selection takes the lower slot on ties for destinations that don't prefix-match either GUA (the broker and the `fd00::` DNS server). So promoting `3fff` to slot 0 biases outbound source selection onto the unreachable address, and DNS replies can't return. This is the mDNS workaround's one real interaction with outbound (the workaround is otherwise inbound-only); it was not packet-captured because the fix makes it non-fatal regardless of the exact source chosen. Worth knowing for a future maintainer: the slot-0 swap is not purely cosmetic to outbound.

**The fix (commit `731c0ae`).** Bracket the two `getaddrinfo()` calls with `esp_task_wdt_delete(NULL)` / `esp_task_wdt_add(NULL)`+reset (this MicroTask runs in `loopTask`), so a stalled cold resolve fails on its own bounded budget and the reconnect path retries, instead of rebooting. ESP32-guarded; no change to the warm-cache or IP-literal fast paths. Verified on metal (`462e6d6`): `3fff`-wins-slot-0 boot no longer reboots; `getaddrinfo` fails bounded, the device stays up (telemetry/MQTT degraded only while the pathological prefix sits in slot 0) and recovers fully once it is gone. This is the WiFi/gateway module, not the OpenEVSE controller that drives the contactor (separate unit over RAPI), so the degraded window does not interrupt a live charging session; the reboot *loop* was the real harm and is removed.
